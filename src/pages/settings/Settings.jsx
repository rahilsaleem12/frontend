...
export default function Settings() {
  ...
  React.useEffect(() => {
    ...
    const handler = (e) => {
      console.log('[SmartPOS] beforeinstallprompt fired');
      e.preventDefault();
      setDeferredPrompt(e);
      setCanInstall(true);
    ...
    window.addEventListener('appinstalled', () => {
      console.log('[SmartPOS] App installed successfully');
      setIsInstalled(true);
      setCanInstall(false);
      setDeferredPrompt(null);
      toast.success('SmartPOS installed successfully!');
    });
  ...
  const handleInstall = async () => {
    if (!deferredPrompt) {
      toast.error('Installation not available on your device. Try on Android Chrome or desktop.');
      console.log('[SmartPOS] deferredPrompt:', deferredPrompt);
      console.log('[SmartPOS] canInstall:', canInstall);
      return;
    }
    ...
    try {
      ...
      console.log('[SmartPOS] Installation outcome:', outcome);
      ...
      console.error('[SmartPOS] Install error:', error);
      toast.error('Installation failed. Try again later.');
    }
  };
  ...
  return (
    <div style={{ maxWidth: '700px' }}>
      ...
      <div className="card">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {/* Install Button - Only show if CAN install */}
          {canInstall && !isInstalled && (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', background: '#FFE5D9', borderRadius: 'var(--radius-md)', border: '1px solid #FF6B35' }}>
              <div>
                <div style={{ fontWeight: 600, color: '#FF6B35', display: 'flex', alignItems: 'center', gap: '6px' }}>
                  ✨ Install SmartPOS
                </div>
                <div style={{ fontSize: '12px', color: '#666', marginTop: '4px' }}>
                  Add to home screen for offline access & quick launch
                </div>
              </div>
              <button 
          ...
          {/* Already Installed - Only show if IS installed */}
          {isInstalled && (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', background: '#F0FFF4', borderRadius: 'var(--radius-md)', border: '1px solid #10B981' }}>
              <div>
                <div style={{ fontWeight: 600, color: '#10B981', display: 'flex', alignItems: 'center', gap: '6px' }}>
                  ✓ App Installed
                </div>
                <div style={{ fontSize: '12px', color: '#666', marginTop: '4px' }}>
                  SmartPOS is installed as a standalone app on your device
                </div>
              </div>
            </div>
          )}

          {/* Not Available - Show if neither can install nor is installed */}
          {!canInstall && !isInstalled && (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', background: '#F0F0F0', borderRadius: 'var(--radius-md)', border: '1px solid #E0E0E0' }}>
              <div>
                <div style={{ fontWeight: 600, color: '#666' }}>📱 Installation Not Available</div>
                <div style={{ fontSize: '12px', color: '#999', marginTop: '4px' }}>
                  Install is available on Android devices or desktop browsers
                </div>
              </div>
            </div>
          )}
          ...
          {/* Cache Management */}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', background: 'var(--bg-tertiary)', borderRadius: 'var(--radius-md)', border: '1px solid #E0E0E0' }}>
            <div>
              <div style={{ fontWeight: 600 }}>🗑️ Clear Cache</div>
              <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '4px' }}>
                Remove offline data & force refresh from server
              </div>
            </div>
            <button 
          ...
          {/* Device Info */}
          <div style={{ padding: '12px', background: '#F8F9FA', borderRadius: 'var(--radius-md)', border: '1px solid #E0E0E0' }}>
            <div style={{ fontSize: '12px', color: '#666' }}>
              <div><strong>Installation Status:</strong></div>
              <ul style={{ margin: '8px 0 0 20px', fontSize: '11px', color: '#999' }}>
                <li>Device Type: <strong>{/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ? 'Mobile' : 'Desktop'}</strong></li>
                <li>Browser: <strong>{navigator.userAgent.includes('Chrome') ? 'Chrome' : navigator.userAgent.includes('Safari') ? 'Safari' : 'Other'}</strong></li>
                <li>Status: <strong>{isInstalled ? 'Installed ✓' : canInstall ? 'Ready to Install' : 'Not Available'}</strong></li>
              </ul>
            </div>
          </div>
        </div>
